resource "aws_instance" "ms_sql" {
  ...
  <powershell>
    ...
    New-ItemProperty -Path 'HKLM:\System\CurrentControlSet\Control\Session Manager\Environment' -Name 'SPLUNK_GATEWAY_URL' -Value http://${aws_lb.collector-lb.dns_name}
